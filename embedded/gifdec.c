...
int read(uint8_t **fd, uint8_t *out, int num)
{
    memcpy(out, *fd, num);
    *fd += num;
    return num;
...
static uint16_t
read_color(uint8_t **fd)
{
    uint8_t bytes[3];

    read(fd, bytes, 3);
    return (((uint16_t)bytes[2] & 0xF8) << 8) | (((uint16_t)bytes[1] & 0xFC) << 3) | (((uint16_t)bytes[0] & 0xF8) >> 3);
}

uint16_t canvas[240 * 240];

gd_GIF *
gd_open_gif(uint8_t *fd)
{
    uint8_t sigver[3];
    ...
    /* Read GCT */
    gif->gct.size = gct_sz;
    for (int i = 0; i < gif->gct.size; i++)
        gif->gct.colors[i] = read_color(&fd);
    gif->palette = &gif->gct;
    gif->bgindex = bgidx;
    gif->canvas = canvas;
    //if (gif->bgindex)
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    while (frm_off < frm_size)
    {
        ...
        for (i = 0; i < str_len; i++)
        {
            ...
            uint16_t color = gif->palette->colors[entry.suffix];
            if (!gif->gce.transparency || entry.suffix != gif->gce.tindex)
                gif->canvas[((gif->fy + y) * gif->width + gif->fx + x)] = color;
...
read_image(gd_GIF *gif)
{
    ...
    if (fisrz & 0x80)
    {
        ...
        for (int i = 0; i < gif->lct.size; i++)
            gif->lct.colors[i] = read_color(&gif->fd);
        gif->palette = &gif->lct;
    }
    else
        gif->palette = &gif->gct;
