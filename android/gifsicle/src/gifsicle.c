...
main(int argc, char *argv[])
{
  ...
  __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE0");

  program_name = Clp_ProgramName(clp);
  __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE1");

  frames = new_frameset(16);
    __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE2");

  initialize_def_frame();
    __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE3");

  Gif_InitCompressInfo(&gif_write_info);
    __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE4");

  Gif_SetErrorHandler(gifread_error);

#if ENABLE_THREADS
  pthread_mutex_init(&kd3_sort_lock, 0);
    __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE5");
  ...
  while (1) {
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE6 %d", opt);
...
particular purpose.\n");
     ...
     default:
      ...
      __android_log_print(ANDROID_LOG_VERBOSE, "--->>", "HERE7");
